...
describe MarkLogic::Queries::RangeQuery do
  describe '#to_s' do
    it 'should handle numeric values correctly' do
      ...
      expect(q.to_s).to eq(%{cts:json-property-range-query("id","=",(123),(),1.0)})
    end

    it 'should handle boolean values correctly' do
      q = MarkLogic::Queries::RangeQuery.new(:id, '=', 'boolean', true)
      expect(q.to_s).to eq(%{cts:json-property-range-query("id","=",(fn:true()),(),1.0)})
    end

    it 'should handle boolean values correctly' do
      q = MarkLogic::Queries::RangeQuery.new(:id, '=', 'boolean', false)
      expect(q.to_s).to eq(%{cts:json-property-range-query("id","=",(fn:false()),(),1.0)})
    end
    ...
    it 'should set the multiple options' do
      ...
      expect(q.to_s).to eq(%{cts:json-property-range-query("id","=",("123"),("collation=http://marklogic.com/collation/codepoint","uncached","synonym"),1.0)})
    end
  end
end
