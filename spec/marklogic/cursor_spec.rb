...
describe MarkLogic::Cursor do
  ...
  describe '#sort' do
    it 'should sort ascending' do
      c = MarkLogic::Cursor.new(@collection, sort: [['age', 1]])
  ...
  describe '#operators' do
    it 'should support EQ' do
      ...
      c = MarkLogic::Cursor.new(@collection, query: @collection.from_criteria(criteria))
  ...
  describe 'each' do
    it 'should work when indexes are provided' do
      ...
      c.each do |doc|
        count += 1
        expect(doc.is_a?(Hash)).to eq true
      end
    ...
    it 'should transform results when a transformer is provided' do
      ...
      criteria = { 'age' => { '$gt' => 3 } }
      c = MarkLogic::Cursor.new(@collection, query: @collection.from_criteria(criteria),
                                             transformer: ->(doc) { @user_class.new(doc['_id'], doc['name'], doc['age'], doc['stuff']) })

      count = 0
      c.each do |doc|
        count += 1
        expect(doc.is_a?(@user_class)).to be true
